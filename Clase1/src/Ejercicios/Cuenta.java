...
(un entero largo), identificación del cliente (otro entero largo),
el saldo actual y el interés anual que se aplica a la cuenta (porcentaje). 
...
public class Cuenta {
    ...
    //actualizarSaldo(): actualizará saldo de la cuenta aplicándole el
    private static class interés {
    ...
    public float actualizarSaldo() {
        float saldo_nuevo;
        saldo_nuevo = saldo_actual + (saldo_actual * ((interes_anual 
                / 100) / 365));
        return saldo_nuevo;
    }
    ...
    void mostrar(double ing, double ret) {
        ...
        System.out.println("Saldo despues del ingreso realizado:" 
                + ingresar(ing));
        System.out.println("Saldo despues del retiro realizado:" 
                + retirar(ret));
